...
<{{ $tag }}
    ...
    @if ($hasFormProcessingLoadingIndicator)
        x-data="(typeof filamentFormButton === 'function') ? filamentFormButton : ({
            isProcessing: false,
        ...
        x-bind:class="{ 'fi-processing': (typeof isProcessing !== 'undefined' && isProcessing) }"
    @endif
    {{
        $attributes
            ->merge([
                ...
                'x-bind:aria-label' => ($labelSrOnly && $hasFormProcessingLoadingIndicator)
                    ? ("(typeof isProcessing !== 'undefined' && isProcessing) ? processingMessage : " . \Illuminate\Support\Js::from(trim(strip_tags($slot->toHtml()))))
                    : null,
            ], escape: false)
            ->when(
                $disabled && $hasTooltip,
...
>
    ...
    @if (! $labelSrOnly)
        @if ($hasFormProcessingLoadingIndicator)
            <span x-show="!(typeof isProcessing !== 'undefined' && isProcessing)">
                {{ $slot }}
            </span>
        @else
            {{ $slot }}
